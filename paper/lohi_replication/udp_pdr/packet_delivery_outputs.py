...
CORE_FILES = {
    ...
    "udp_rtt_summary_by_algorithm.csv": (
        "Focus-flow propagation-only and queue-aware estimated RTT summary.",
        "udp_focus_rtt_timeseries.csv",
        True,
    ),
    "udp_rtt_components_by_algorithm.csv": (
        "Focus-flow estimated RTT component distributions.",
        "udp_focus_rtt_timeseries.csv",
        True,
    ),
    "focus_rtt_over_time.png": (
        "Propagation-only focus-flow RTT over time.",
        "udp_focus_rtt_timeseries.csv",
        True,
    ),
    "focus_queue_aware_rtt_over_time.png": (
        "Queue-aware estimated focus-flow RTT over time.",
        "udp_focus_rtt_timeseries.csv",
        True,
    ),
    "rtt_summary_by_algorithm.png": (
        "Mean propagation-only and queue-aware estimated RTT by algorithm.",
        "udp_rtt_summary_by_algorithm.csv",
        True,
    ),
    "rtt_component_breakdown.png": (
        "Mean estimated RTT component breakdown by algorithm.",
        "udp_rtt_components_by_algorithm.csv",
        True,
    ),
    "focus_forward_reverse_hop_count.png": (
        "Focus-flow forward and reverse route hop counts over time.",
        "udp_focus_rtt_timeseries.csv",
        True,
    ),
}
...
DIAGNOSTIC_FILES = {
    ...
    "lhtr_traffic_light_color_summary.csv": "LHTR traffic-light color counts by routing snapshot.",
    "lhtr_br_sbr_summary.csv": "LHTR BR/SBR selection counts by routing snapshot.",
    "lhtr_decision_reason_summary.csv": "LHTR decision reasons aggregated over the run.",
    "lhtr_fstate_decision_consistency.csv": "LHTR selected-next-hop and final-fstate consistency checks.",
    ...
    "udp_focus_rtt_timeseries.csv": (
        "Per-sample propagation, transmission, queue-delay, and RTT estimates."
    ),
    "udp_focus_path_timeseries.csv": (
        "Per-sample forward/reverse path nodes, links, and replay status."
    ),
    "udp_rtt_diagnostics.txt": (
        "RTT assumptions, focus-pair source, and queue-data quality diagnostics."
    ),
}
...
def ensure_standard_layout(comparison_dir):
    for dirname in ["core", "diagnostics", "legacy", "graphical_routes"]:
...
def write_result_guide(comparison_dir):
    ...
    with open(path, "w") as f_out:
        f_out.write(
            ...
            "## Estimated RTT and Routes\n\n"
            "`core/udp_rtt_summary_by_algorithm.csv` and "
            "`core/udp_rtt_components_by_algorithm.csv` summarize the focus-flow "
            "propagation-only and queue-aware RTT estimates. The detailed path, "
            "component, queue-source, and missing-data fields are in "
